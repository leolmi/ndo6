...
/**
 * User must be authenticated and owner or invitated on map
 * @returns {Namespace|Authenticator|app}
 */
function isAuthenticatedOnMap() {
  return compose()
    // Validate jwt and attach user to request
    .use(isAuthenticated())
    // Validate user on map
    .use(function (req, res, next) {
      Map.findById(req.params.id, function (err, map) {
        ...
        Invitation.find(filter, function (err, invitation) {
          if (err) return next(err);
          if (!invitation) return res.send(401);
          next();
        });
      });
    });
...
exports.isAuthenticatedOnMap = isAuthenticatedOnMap;
